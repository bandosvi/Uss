...
const QZ = [
  ...
  {id:"this-or-that",title:"This or That",emoji:"🎲",color:GOLD,desc:"Fun choices",time:"4 min",type:"this-or-that",
...
export default function Us() {
  ...
  const answer = (val) => {
    ...
    else { alert("Quiz completed! ❤️"); setActiveQuiz(null); }
  };
  ...
  return (
    <div style={{minHeight:"100vh", background:DARK, color:"#f0e8f0", fontFamily:"system-ui", display:"flex", flexDirection:"column"}}>
      ...
      <div style={{padding:"14px 16px", background:SURF, borderBottom:`1px solid ${BDR}`, position:"sticky", top:0, zIndex:10, display:"flex", justifyContent:"space-between", alignItems:"center"}}>
        ...
        <div style={{fontSize:15}}>{me ? `${me.name} ${partner ? `♥ ${partner}` : "♥ Waiting"}` : ""}</div>
      </div>
      ...
      <div style={{flex:1, overflowY:"auto", padding:"20px 16px"}}>
        ...
        {phase === "paired" && nav === "home" && (
          <div>
            ...
            <div style={{display:"grid", gap:16, marginTop:30}}>
              ...
              <button onClick={()=>setNav("cal")} style={{padding:24, background:CARD, borderRadius:20, fontSize:18, textAlign:"left", border:`1px solid ${BDR}`}}>📅 Calendar</button>
        ...
        {nav === "rescue" && (
          <div>
            ...
            <button onClick={()=>{const t=prompt("Topic?"); if(t){alert("Case opened! Dr. Rescue is thinking...");}}} style={{padding:20, background:CARD, borderRadius:20, width:"100%", marginTop:20}}>Open New Case</button>
          </div>
        )}
      </div>
